...
public class DBTasksHelper {
     ...
     * 此方法用來設定準備查詢的資料範圍。
     *
     * @param projection    類型為String[]。表示資料表中欲選擇的欄位--傳入null可回傳全部--不過如需回傳全部建議不傳用我的預設值，或匯入"ColumnTask.PROJECTION"這組我預先定義的全選陣列。
     * @param selection     類型為String。即SQL之(where=..)語句，如直接傳入5即等於(where==5)；可傳入＝?搭配selectionArgs使用。例如: <br>
     *                      selection="City=高雄";
     * @param selectionArgs 類型為String[]。搭配selection與?使用，表示欲查詢的元素(s)。例如：<br>
     *                      selection="City=?"; <br>
     *                      selectionArgs={高雄,台北};
     * @param shortOrder    類型為String。表示欲排序的依據與順序。shortOrder="欄位名 [ASC,DESC]";
     * @return 回傳一個可以套進各式Adapter的Cursor資料物件。
     * @throws null
     */
